...
- **Known fill-ins for the executor (not placeholders — concrete instructions):** the `merchantQuoteInput()` test helper body (mirror an existing order-create test's input array) and the exact `QuoteToken` verify API must be confirmed against the codebase before the test is committed.
- **Order-create auth guard:** there is **no** phone-verified middleware on `POST /api/orders`; phone verification + `canCreateOrders()` live in `CreateOrderRequest::authorize()`, which `StoreMerchantOrderRequest::authorize()` replicates (Task B.3). Do not add a middleware for this.
