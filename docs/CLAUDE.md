...
**Status:** Schema phase (1–9) ✅ done. **Auth ✅. Driver onboarding ✅. Order lifecycle A+B ✅. Sub-project C pre-pickup tail ✅ (Slice 10). Sub-project D failed delivery + return-to-office ✅ (Slice 11). Settlement & seller payouts ✅ (milestone 2026-05-17). Staff CRUD ✅ (milestone 2026-05-20, Slices A+B). Internal-ID exposure remediation ✅ (PR #5, merged 2026-06-02). Real-time / Reverb ✅ (milestone 2026-06-02). Account moderation ✅ (milestone 2026-06-03, PRs #9/#10). Test infrastructure ✅ (milestone 2026-06-04). Merchant deliveries ✅ (sub-project E, 2026-06-12, PRs #14/#15). Dashboard Support A ✅ (admin-only backend foundations, 2026-06-18, PRs #19/#20 — §17.18). Dashboard Support B ✅ (Overview, finance report, staff activity, notification-pref editing, 2026-06-23, PR #22 — §17.19). Dashboard Operations UX Support ✅ (map zone GeoJSON + human-readable order numbers, 2026-07-19, PRs #27/#28 — §17.20).** Cash loop closed end-to-end; admin staff account management live; real-time push live across order/driver/user channels; admin can suspend/ban/reinstate any account with audited reasons; all e2e smokes run as Pest tests in CI; merchants onboarded by admins create shop→customer orders that settle through the existing payout pipeline; the admin API surface the internal dashboard binds to is complete (413 Pest tests). Orders now carry an immutable human-readable `order_number` beside the ULID `id`, and `/admin/map/zones` serves the zone polygons the dashboard's map overlay draws.

**Frontend:** the internal dashboard is a **separate repo** (`delivary-dashboard`, React 18 + TS + Vite — the older "Vue 3" note is superseded). Slice 0 (auth foundation) and Slice 1 (Operations) are merged there.

**Next: Zone Management** — service areas, regions, and offices are still **seed-only**; there is no admin write path for any of them, yet `regions.base_fee` prices every order and `regions.office_id` drives finance attribution and driver region assignment. Planned as two slices: a backend safety/CRUD milestone (deactivate-only lifecycle, restrictive FKs, PostGIS validity/containment/non-overlap validation, full-snapshot revisions with restore, optimistic concurrency) followed by a MapLibre + Terra Draw zone editor in the dashboard. Admin-created orders remain deferred by product decision.
...
### Next Steps (in order)
1. **Internal dashboard frontend** — Vue 3 + TS + Vite (separate repo), built against the now-stable admin API (Dashboard Support A+B). Its own brainstorm → spec first.
2. **Convert `realtime-smoke.php` to Pest** (`RealtimeSmokeTest`) — deferred from the test-infrastructure milestone.
3. **Admin-created orders** — still deferred by product decision; revisit only if operations needs back-office order entry.
