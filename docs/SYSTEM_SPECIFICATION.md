...
### 17.15 Account Moderation milestone (2026-06-03) ✅

Admin-only account-level moderation (suspend/ban/reinstate) for **any** user, with structured reasons and an append-only audit trail. Built per `docs/superpowers/specs/2026-06-03-account-moderation-design.md` on the parallel-worktree workflow: Claude = Slice A (domain core), Codex = Slice B (HTTP + StaffService delegation); each reviewed the other's PR (PR #9 core, PR #10 http). Implements the "suspensions" responsibility from §2.5 and delivers the staff-action audit table deferred from Staff CRUD.

Moderation acts on the **`AccountStatus`** axis (can this person use the platform at all) — distinct from the operational `DriverStatus` axis (§9.1) and strike system (§9.6), which are untouched. `AccountModerationService` is the sole authority for manual `Active ⇄ Suspended/Banned`: public methods open one transaction, `lockForUpdate()`+reload the target, run guards (self / last-active-admin / valid-transition) against that committed state, then write status + revoke tokens + run the targeted cascade + append an audit row (Critical Rule 3 — no check-then-act on stale instances; the last-admin count locks the other admin rows before counting). **Targeted cascade:** drivers are forced `offline` (dropping them from `BroadcastService::eligibleDriversFor`) without touching `DriverStatus`, and a live delivery is left for support — never auto-cancelled; staff keep their office assignments.

`StaffService::suspend/reinstate/deactivate` now delegate the status+token+audit to `AccountModerationService::apply()` (keeping their own `StaffErrorCode` guards, so the staff API contract is unchanged) — every staff suspension is now audited. **Reinstate respects finance:** a reinstated user who still owes fees (`User::hasOutstandingFees()` — MVP: driver `debt_balance > 0`) lands in `SuspendedUnpaidFees`, making this milestone the first writer of that previously-dead state.

**Endpoints shipped (5 routes, admin-only, double-gated `role:admin` + `ModerationPolicy`, `public_id`-bound):** `GET /api/admin/users/lookup?phone=` (thin identity payload — the minimal lookup mirrors `office/*/lookup`; full directory deferred to the dashboard), `POST /api/admin/users/{user}/{suspend,ban,reinstate}` (required `reason_code` + `detail`), `GET /api/admin/users/{user}/moderation-history` (paginated). `throttle:moderation` (30/min/admin).

**Schema delta:** new append-only `account_moderation_actions` table (`public_id`, target `user_id`, `actor_id`, `action`, `reason_code`, `detail`, `from_status`/`to_status` snapshots, `created_at`). New enums `ModerationAction`/`ModerationReason`/`ModerationErrorCode`; `ModerationException` rendered via `bootstrap/app.php`.

**Cross-review caught + fixed:** (Claude→Codex) self-moderation returned `403` from the policy, shadowing the `422 CANNOT_MODERATE_SELF` contract → policy reduced to an admin gate, service owns the self guard. (Codex→Claude) guards/`from_status` ran on stale, unlocked model state → wrapped in a locking transaction (above).
